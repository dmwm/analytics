...
class IPResolver:
  ...
  def query(self):
    ...
    @return Dictionary of IP address strings to `IPInfo` objects, one
    for every unique IP address earlier issued with `submit()`.
    """
  ...
  def _asn_lookup_2(self, info, task, tasks):
    ...
    def responder(answer, addr):
      debug("IP2INFO", 3, "routeviews result %s %s %s", addr, info.ip, answer)
      self._tick_stats(answer[0])
      task.ongoing = False
  ...
  def _asn_lookup_cc(self, info, task, tasks):
    ...
    def responder(answer, addr):
      debug("IP2INFO", 3, "cc result %s %s %s", addr, info.asn.asn, answer)
      self._tick_stats(answer[0])
      task.ongoing = False
  ...
  def _wild_lookup(self, info, task, tasks):
    ...
    def responder(hostname):
      debug("IP2INFO", 3, "wild result %s %s", info.ip, hostname)
      task.ongoing = False
      if hostname != None:
        task.done = True
  ...
  for ip in sys.argv[1:]:
    ...
    print "%-40s %-10s %-6s %-25s %-14s %s %s %s" % \
      ("ip=%s [%s]" % (i.ip, i.cidr),
       ...
       "cc=%s" % i.geoip.cc,
       "domain=%s" % i.domain,
       "pos=%s" % ((i.geoip.lat != None and "%.2f,%.2f" % (i.geoip.lat, i.geoip.long)) or ""),
